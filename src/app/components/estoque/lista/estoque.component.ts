...
@Component({
  selector: "page-estoque-component",
...
export class EstoqueComponent {
  ...
  constructor(
    private _service: EstoqueService,
    ...
    this._service
      ...
      .then((dados) => {
        if (dados.length > 0) {
          ...
          this.pararExec = false;
          this._util.configurarImgEstoque(this.estoque);
          this.setEstoqueParams(this.estoque);
        } else {
          this.estoque = null;
        }
      })
  ...
  atualiza(estoqueAtual: EstoqueAtual) {
    this.estoque = estoqueAtual.estoque.sort(
      (a, b) => Number(a.tanque) - Number(b.tanque)
    );
    this.dataHora = estoqueAtual.dataHora;
  }

  getImg(estoque: Estoque) {
    return UtilService.getImgEstoque(estoque);
  }

  getComb(estoque: Estoque) {
    return UtilService.getNomeComb(estoque.combustivel);
  }

  getClass(est: Estoque) {
    return UtilService.getClassEstoque(est.combustivel);
  }

  async setEstoqueParams(est) {
    est.forEach((e) => {
      let main = e.color;
      let back = e.colorBack;
      let id = e.elementId;
      let percent = e.porcentagem;
      setTimeout(() => {
        this.desenharTanques(main, back, percent, id);        
      }, 100);
    });
  }
  ...
  desenharTanques(liquidColor, liquidBackgroundColor, percent, id) {
  ...
  /**
   * Funcao que desenha o liquido para representar o estoque de tanques
  ...
  fluidMeter() {
    ...
    var drawFluidLayer = (layer, dt) => {
      ...
      if (layer.horizontalSpeed > 0) {
        layer.horizontalPosition > Math.pow(2, 53)
          ? 0
